...
{{--                        <h1 class="display-3 text-white" data-toggle="modal" data-target="#modal-restaurant-info" style="cursor: pointer;">{{ $restorant->name }}</h1>--}}
                        <h1 class="display-3 text-white" data-toggle="modal"  style="cursor: pointer;">{{ $restorant->name }}</h1>
                    ...
                    @foreach ($category->items as $item)
                        ...
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6">
                            <div class="strip">
                                @if(!empty($item->image))
                                <figure>
                                    <a onClick="setCurrentItem({{ $item->id }})" href="javascript:void(0)"><img src="{{ asset('/') }}{{ $item->logom }}" loading="lazy" data-src="{{ asset('/') }}/{{ config('global.restorant_details_image') }}" class="img-fluid lazy" alt=""></a>
                                </figure>
                                @endif
