...
@section('content')
    ...
    <section class="section-profile-cover section-shaped grayscale-05  d-lg-block d-lx-block" style="height: 260px">
        <!-- Circles background -->
        <img class="bg-image" loading="lazy" src="{{ asset('/') }}/{{ $restorant->coverm }}" style="width: 100%;">
        <!-- SVG separator -->
...
{{--                        <p>@if(!empty($openingTime) && !empty($closingTime))  <i class="ni ni-watch-time"></i>--}}
{{--                            <span>{{ $openingTime }}</span> - <span>{{ $closingTime }}</span>--}}
{{--                            | @endif  @if(!empty($restorant->address))<i class="ni ni-pin-3"></i></i> <a target="_blank"--}}
                            ...
                            @if($item->available == 1)
                                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6">
                                    <div class="strip">
                                        @if(!empty($item->image))
                                            <figure>
                                                <a onClick="setCurrentItem({{ $item->id }})"
                                                   href="javascript:void(0)"><img
                                                        src="{{ asset('/') }}{{ $item->logom }}" loading="lazy"
                                                        data-src="{{ asset('/') }}/{{ config('global.restorant_details_image') }}"
